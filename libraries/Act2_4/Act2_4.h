...
class Act2_4{
	...
	protected:
		...
		double pctg_error = 50; // Percentage error 
	...
	public:
		...
		{		
			...
			if(target_speed_check.isMinChekTimeElapsedAndUpdate())
			{
				...
				plot_and_replay(pctg_error, curr_speed);
			}
...
//				}
		...
		void plot_and_replay(double pctg_error, double curr_speed)
		{
			int settled_count = 20;
	
			// Error bars
			double min_error = target_speed * (1-(pctg_error/100));
			double max_error = target_speed * (1+(pctg_error/100));

			if ((pr_counter < settled_count) && (HBmotor.isStarted()))
			{
				// Store current speed into array value and print 
				store[i] = curr_speed;
				
				// Print array in serial monitor
				Serial.print(i);
				Serial.print("   ");
				...
				// If current speed is within error bars
				if ((curr_speed <= max_error) && (curr_speed >= min_error))
				{
					pr_counter++;
				}
			...
			// Replay data
			if (pr_counter == settled_count)
			{
				Serial.println("Speed settled - recording ended");
				Serial.println("Replay data:");
				Serial.print("Sample   ");
				Serial.println("RPM");

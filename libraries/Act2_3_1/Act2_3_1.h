	...
	public:
		...
		{		
			...
			if(target_speed_check.isMinChekTimeElapsedAndUpdate())
			{
				...
				RiseTime(target_speed, curr_speed);
		...
		void RiseTime(int target_speed, int curr_speed)
		{
			// impossible to get a time for exactly 90% as in order to do so,
			// an infinite number of samples would need to be sampled.
			int min_rise_val, max_rise_val;	
			min_rise_val = 0.88 * target_speed;
			max_rise_val = 0.92 * target_speed;
			
			
			if ((HBmotor.isStarted()) && (!check))
			{			
				if(curr_speed < min_rise_val)
				{
					rtcounter += 0.01;
				}
				else if((curr_speed >= min_rise_val) && (curr_speed <= max_rise_val))
				{	
					Serial.print("Time to 90% rise time = ");
